	...
	app.onactivated = function (args) {
		if (args.detail.kind === activation.ActivationKind.launch) {
			...
			webView.addEventListener("MSWebViewNavigationStarting", () => {
			    ...
			    webView.addWebAllowedObject("NotifyComponent", NotifyComponent);
			})
			...
			saveButton.addEventListener("click", () => {
			    ...
			    localFolder.createFolderAsync("TempDir", Windows.Storage.CreationCollisionOption.openIfExists).then((folder) => {
			        ...
			        return Windows.Storage.FileIO.writeTextAsync(file, "<!DOCTYPE html><html><head><script src='ms-appx-web:///inWebView.js'></script></head><body>Inside WebView <button id='doSomething'>Send Message</button><button id='callComponent'>Call Component</button></body></html>");
			    }).then(() => {
